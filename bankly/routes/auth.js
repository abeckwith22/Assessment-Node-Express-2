...
router.post('/register', async function(req, res, next) { // - [x] CHECKED
  try {
...
router.post('/login', async function(req, res, next) { // - [x] CHECKED
  try {
    const { username, password } = req.body;
    // FIXES BUG #1
    let user = await User.authenticate(username, password);
    const token = await createTokenForUser(username, user.admin);
