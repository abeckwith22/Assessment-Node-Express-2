...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #1
  test("should raise a 401 error if incorrect username/password", async function(){
    const response = await request(app)
    .post("/auth/login")
    .send({
      username: "u1",
...
describe("GET /users", function() {
  ...
  // TESTS BUG #2 for slightly wrong-information
  test("should only list basic information (username, first_name, last_name)", async function(){
    const response = await request(app)
    .get("/users")
    .send({ _token: tokens.u1 });
    expect(response.body.users[0].username).toEqual('u1');
    expect(response.body.users[0].first_name).toEqual('fn1');
    expect(response.body.users[0].last_name).toEqual('ln1');
    expect(response.body.users[0].email).toBe(undefined); // email shouldn't be listed in this route, so searching for it should return undefined.
  });
});
...
describe("GET /users/[username]", function() {
  ...
  test("should return 404 error if user can't be found", async function(){
    const response = await request(app)
...
describe("PATCH /users/[username]", function() {
  ...
  test("should patch data if matching user", async function() {
    const response = await request(app)
      .patch("/users/u2")
      .send({ _token: tokens.u2, first_name:"new-fn2" }); // u1 is not admin but matching user
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u2",
      first_name: "new-fn2",
...
describe("DELETE /users/[username]", function() {
  ...
  // thought test wasn't rigid enough if we didn't test for 404 error
  test("should return 404 err if user not found", async function(){
    const response = await request(app)
      .delete("/user/not-a-user")
      .send({ _token: tokens.u3}); // u3 is admin
    expect(response.statusCode).toBe(404);
  });
});
