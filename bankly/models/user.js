...
/** Register user with data. Returns new user data. */
  ...
  static async get(username) {
    ...
    if (!user) {
      // FIXES BUG #3 add a throw statement on new ExpressError()
  ...
  static async update(username, data) {
    // FIXES BUG #5 
    if(Object.keys(data).length === 0 && data.constructor === Object){ // finally if data is an empty obj (meaning either user sent no data or incorrect/not-allowed data) will throw an invalid data.
      throw new ExpressError('Invalid data.', 401);
    }
